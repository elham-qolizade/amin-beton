...
function AddOrderPaymentForm({ orderId, onSuccess }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://amin-beton-back.chbk.app/api/order-management/${orderId}/add-order-payments/`,
        {
          ...
          headers: {
            ...
            // "Content-Type": "multipart/form-data" حذف شد
          },
          body: formData,
        }
      );
      ...
      // بررسی وضعیت پاسخ
      if (!response.ok) {
        const errorText = await response.text(); // دریافت محتوای پاسخ به صورت متن
        try {
          const errorData = JSON.parse(errorText); // تلاش برای تبدیل به JSON
