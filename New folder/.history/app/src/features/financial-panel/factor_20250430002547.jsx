...
function InvoicesListWithForm({ orderDetails }) {
  ...
  const handleCreateInvoice = async (e) => {
    ...
    try {
      ...
      // ✅ خواندن صحیح توکن access از localStorage
  ...
  const handleCreateInvoice = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        is_paid: false, // اگر بخواهی این فاکتور پرداخت‌شده تلقی نشه
      };
      ...
      if (response.status === 201) {
        ...
        setInvoices((prev) => [...prev, data]);
        setPrice("");
        setInvoiceFile(null); // فایل دیگه استفاده نمی‌شه، ولی نگهش داری مشکلی نیست
      } else {
