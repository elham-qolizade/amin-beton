...
const PumpPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(
        "https://amin-beton-back.chbk.app/api/orders/list",
        {
          headers: {
      ...
      setOrders(response.data); // اگر لیست سفارشات در state به اسم orders نگه‌داری می‌کنی
    } catch (error) {
  ...
  if (response.status === 200) {
    console.log("✅ Server Response:", response.data);
    toast.success("✔ سفارش با موفقیت ارسال شد!");
    await fetchOrders(); // فقط لیست سفارشات رفرش میشه ✅
 // ✅ اینجا لیست سفارشات فقط آپدیت میشه
            ...
            {selectedPumps.map((pump) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-2">{pump.pump?.title || "پمپ نامشخص"}</div>
                  <div>تعداد پمپ: {pump.count}</div>
