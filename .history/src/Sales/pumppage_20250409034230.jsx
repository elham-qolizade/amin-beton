        ...
        <div className="mb-8">
          ...
          <div className="flex flex-row flex-wrap gap-6 mt-4">
            {selectedPumps.map((pump) => {
              // چک می‌کنیم که پمپ داده معتبر باشد
              ...
              return (
                <div key={pump.id}>
                  <h3>{pump.pump.title}</h3>
                  {subPumpsForThisPump.length > 0 ? (
                    <ul>
                      {subPumpsForThisPump.map((subPump) => (
                        <li key={subPump.id}>{subPump.title}</li>
                      ))}
                    </ul>
                  ) : (
                    <p>هیچ زیرپمپی برای این پمپ وجود ندارد.</p>
                  )}
                </div>
              );
            })}
