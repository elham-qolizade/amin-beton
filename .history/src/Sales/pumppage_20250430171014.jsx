...
const PumpPage = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen p-6 text-white bg-Bokara-Grey">
      <div className="container w-full max-w-5xl p-8 bg-gray-900 rounded-2xl shadow-lg">
        <div className="mb-8">
        ...
        {tempSelectedPumps.some((pump) => pump.countable) && (
          <div className="flex items-center gap-3 mt-6">
            ...
            <input
              ...
              className="w-24 p-2 text-black rounded-md border border-gray-400 focus:ring-2 focus:ring-yellow-400 focus:outline-none"
            />
          </div>
        )}

        <div className="flex justify-center gap-6 mt-10">
          <Button className="px-10 py-2 rounded-xl bg-yellow-500 hover:bg-yellow-400 text-black shadow-md">
            افزودن پمپ‌ها
          </Button>
        </div>
        ...
        <div className="mt-12">
          ...
          <div className="flex flex-wrap gap-6">
            {selectedPumps.map((pump) => {
              if (!pump.pump || !pump.pump.title) return null;
              const subPumpsForThis = pump.sub_pump ? [pump.sub_pump] : [];

              return (
                <div
                  key={pump.id}
                  className="flex flex-col items-center justify-center w-40 gap-2 px-4 py-4 text-center border border-yellow-400 bg-gray-800 rounded-xl shadow-md"
                >
                  <div className="w-4 h-4 mb-2 rounded-full bg-yellow-400"></div>
                  <div className="mb-1 font-semibold">{pump.pump?.title}</div>
                  <div className="text-sm">تعداد: {pump.count}</div>
                  {subPumpsForThis.length > 0 && (
                    <div className="text-xs text-gray-300">
                      {subPumpsForThis.map((sp) => (
                        <div key={sp.id}>زیرپمپ: {sp.title}</div>
                      ))}
                    </div>
                  )}
                  <button
        ...
        <div className="flex justify-end mt-12">
          <Button
            className="px-10 py-2 rounded-xl bg-green-500 hover:bg-green-400 text-black shadow-md"
            onClick={() => {
              handleAddAndSubmit();
              navigate(`/VibratorPage/${orderId}`);
            }}
          >
            ادامه
          </Button>
