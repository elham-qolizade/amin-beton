...
function SaleProject() {
  ...
  const [concreteResistances, setConcreteResistances] = useState([]);
  ...
  // ارسال فرم سفارش
  const selectedOption = resistanceOptions.find(
    (o) => o.value.id === newOrder.concrete_resistance_class?.id
  );
  ...
  const resistanceOptions = concreteResistances.map((cr) => ({
    value: cr, // خود شیء کامل
    label: cr.name, // یا هر فیلد نمایشی
  }));
