...
function SaleProject() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const concreteOptions = typesRes.data.map((item) => ({
          value: item.id,
          label: item.title,
        }));

        const pouringOptions = pouringRes.data.map((item) => ({
          value: item.id,
          label: item.title,
        }));

        const resistanceOptions = resistanceRes.data.map((item) => ({
          value: item.id,
          label: item.title,
        }));

        setConcreteTypes(concreteOptions);
        setConcretePouringTypes(pouringOptions);
        setResistanceClasses(resistanceOptions);

        // مقداردهی پیش‌فرض اگر newOrder مقدار نداشته باشد
        setNewOrder((prev) => ({
          ...prev,
          concrete_type: prev.concrete_type || concreteOptions[0]?.value || "",
          concrete_pouring_type:
            prev.concrete_pouring_type || pouringOptions[0]?.value || "",
          concrete_resistance_class:
            prev.concrete_resistance_class || resistanceOptions[0]?.value || "",
        }));
      } catch (err) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const options = response.data.map((item) => ({
          value: item.id,
          label: item.title, // 👈 اصلاح شد
        }));

        console.log("✅ Mapped options with label:", options);

        setConcreteTypes(options);

        setNewOrder((prev) => ({
          ...prev,
          concrete_type: prev.concrete_type || options[0]?.value || "",
        }));
      } catch (err) {
        toast.error("❌ خطا در گرفتن لیست بتن‌ها");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (selectedDay) {
      ...
      // وقتی تاریخ انتخاب شد، مودال ساعت باز شود
    }
  }, [selectedDay]);
  const errorMessages = {
  ...
  const handleConfirmTime = (time) => {
    if (!selectedDay) {
      setErrorMessage("❌ لطفاً ابتدا تاریخ را انتخاب کنید.");
      setIsModalOpen(true);
      setSelectedTime(""); // پاک کردن ساعت انتخاب شده اگر بود
      return;
    }
    ...
    // ادامه اعتبارسنجی ساعت و ثبت تاریخ و ساعت
    const { gy, gm, gd } = jalaali.toGregorian(
      selectedDay.year,
      selectedDay.month,
      selectedDay.day
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedTime) {
      setErrors((prev) => ({
        ...prev,
        selectedTime: "❌ لطفاً ساعت تحویل را انتخاب کنید.",
      }));
      toast.error("❌ لطفاً ساعت تحویل را انتخاب کنید.");
      formValid = false;
    } else {
    ...
    if (!selectedDay) {
      setErrors((prev) => ({
        ...prev,
        selectedDay: "⚠ لطفاً تاریخ تحویل را انتخاب کنید.",
      }));
      toast.error("⚠ لطفاً تاریخ تحویل را انتخاب کنید.");
      formValid = false;
    } else {
  ...
  const concreteOptions = concreteTypes.map((type) => ({
    value: type.id,
    label: type.title,
  }));
  const resistanceOptions = resistanceClasses.map((cls) => ({
    value: cls.id,
    label: cls.title,
  }));
  const pouringOptions = concretePouringTypes.map((pouring) => ({
    value: pouring.id,
    label: pouring.title,
  }));
  // اگر لودینگ بود...
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-100 bg-Bokara-Grey">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        {/* نوع بتن */}
        <div>
          <label className="block text-white mb-1">نوع بتن</label>

          <Select
            placeholder=""
            options={concreteTypes}
            value={
              concreteTypes.find((o) => o.value === newOrder.concrete_type) ||
              null
            }
            onChange={(selected) =>
              setNewOrder((prev) => ({
                ...prev,
                concrete_type: selected?.value || "",
              }))
            }
            ...
            styles={{
              ...
              option: (base, state) => ({
                ...base,
                backgroundColor: state.isFocused ? "#ffd700" : "#1c1c1c",
                color: "white",
                direction: "rtl",
                fontSize: "0.875rem",
              }),
          ...
          {errors.concrete_type && (
            <p className="text-red text-sm mt-1">{errors.concrete_type}</p>
          )}
        </div>

        {/* مقطع بتن‌ریزی */}
        <div>
          <label className="block mb-1 text-white">مقطع بتن‌ریزی</label>
          <Select
            placeholder=""
            options={concretePouringTypes}
            value={
              concretePouringTypes.find(
                (o) => o.value === newOrder.concrete_pouring_type
              ) || null
            }
            onChange={(selected) =>
              setNewOrder((prev) => ({
                ...prev,
                concrete_pouring_type: selected?.value || "",
              }))
            }
        ...
        {/* رده مقاومت بتن */}
        <div>
          <label className="block mb-1 text-white">رده مقاومت بتن</label>
          <Select
            placeholder=""
            options={resistanceClasses}
            value={
              resistanceClasses.find(
                (o) => o.value === newOrder.concrete_resistance_class
              ) || null
            }
            onChange={(selected) =>
              setNewOrder((prev) => ({
                ...prev,
                concrete_resistance_class: selected?.value || "",
              }))
            }
            className="text-right mb-4"
            styles={{
        ...
        <div className="flex flex-col items-center justify-center gap-4 md:flex-row ">
          <div className="flex items-center  flex-col justify-center ">
            ...
            />
            {errors.selectedDay && (
              <p className="text-red text-sm">{errors.selectedDay}</p>
            )}
          </div>

          <div className="flex flex-col items-center justify-center mt-6 rounded-lg shadow-lg bg-Bokara-Grey">
            <Time
              type="time"
              value={selectedTime}
              onChange={(newTimeStr) => {
                setSelectedTime(newTimeStr);
                handleConfirmTime(newTimeStr); // اعتبارسنجی و ثبت خودکار
              }}
              className="w-full p-2 border border-gray-300 rounded"
            />
            {errors.selectedTime && (
              <p className="text-red mt-2 text-sm">{errors.selectedTime}</p>
            )}
          </div>
        </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="relative z-50 p-6 border rounded-lg bg-Bokara-Grey border-School-Bus">
        ...
        <div className="flex items-center justify-center w-full ">
          <ButtonProject type="submit" className="w-full py-2 mt-8 md:px-40">
            ادامه
          </ButtonProject>
        </div>
      </form>
