...
const VibratorPage = () => {
  ...
  const handleVibratorSelection = (id) => {
    ...
    const handleAddVibrators = () => {
      ...
      const newFormattedSubVibrators = tempSelectedSubVibrators.map(
        (subVib) => ({
  ...
  const handleSubmitOrder = async () => {
    console.log("📦 ارسال سفارش برای orderId:", orderId);
    const token = localStorage.getItem("accessToken");
    ...
    // ساخت newVibrators و newSubVibrators از temp ها
    const newVibrators = tempSelectedVibrators.map((v) => ({
      ...v,
      count: v.count || 1,
    }));
    const newSubVibrators = tempSelectedSubVibrators.map((sv) => ({
      ...sv,
      count: sv.count || 1,
    }));
    ...
    // بعد از موفقیت در ارسال:
    if (response.status === 200) {
      toast.success("✔ سفارش با موفقیت ارسال شد!");

      // افزودن به لیست نهایی برای نمایش در UI
      setSelectedVibrators((prev) => [...prev, ...newVibrators]);
      setSelectedSubVibrators((prev) => [...prev, ...newSubVibrators]);
  ...
  const handleAddAndSubmit = async () => {
    ...
    // به‌روزرسانی وضعیت‌ها بلافاصله قبل از ارسال به سرور
    setSelectedVibrators((prev) => [...prev, ...newVibrators]);
    setSelectedSubVibrators((prev) => [...prev, ...newSubVibrators]);
    ...
    <div className="flex flex-col min-h-screen p-6 text-white bg-Bokara-Grey">
        ...
        {showVibratorSelection && (
          <div className="mb-8">
            ...
            <div className="flex justify-center gap-6 px-10 mt-10">
              <Button className="px-10" onClick={handleSubmitOrder}>
                افزودن ویبراتور
              </Button>
            </div>
          </div>
      ...
      <div className="flex flex-row flex-wrap gap-6 mt-4">
        {selectedVibrators.map((vibrator) => {
          ...
          const subVibratorsForThis = vibrator.sub_vibrator ? [vibrator.sub_vibrator] : [];
