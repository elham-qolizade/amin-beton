  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // تبدیل داده‌ها برای react-select
        ...
        // مقدار پیش‌فرض اگر خالی بود
        setNewOrder((prev) => ({
          ...prev,
          concrete_type:
            prev.concrete_type || (concreteOptions[0]?.value ?? ""),
          concrete_pouring_type:
            prev.concrete_pouring_type || (pouringOptions[0]?.value ?? ""),
          concrete_resistance_class:
            prev.concrete_resistance_class ||
            (resistanceOptions[0]?.value ?? ""),
        }));
      } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        ...
        <div>
          ...
          <Select
            ...
            value={concreteOptions.find(
              (o) => o.value === newOrder.concrete_type
            )}
            onChange={(selected) =>
              handleChange("concrete_type", selected?.value)
            }
