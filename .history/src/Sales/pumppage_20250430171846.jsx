...
const PumpPage = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen p-6 text-white bg-Bokara-Grey">
      <div className="container w-full max-w-5xl p-8 bg-gray-100 rounded-2xl shadow-lg">
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-gray-800">پمپ‌ها</h2>
            ...
            {pumps.map((pump) => {
              ...
              return (
                <div
                  ...
                  } rounded-xl px-4 py-4 cursor-pointer transition-transform duration-300 hover:shadow-lg hover:scale-105`}
        ...
        {tempSelectedPumps.some((pump) => pump.countable) && (
          <div className="flex items-center gap-2 mt-4">
            ...
            <input
              ...
              className="w-24 p-2 text-gray-800 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        )}

        <div className="flex justify-center gap-6 px-10 mt-10">
          <Button className="px-10 bg-blue-600 hover:bg-blue-700 text-white">
            افزودن پمپ‌ها
          </Button>
        </div>
          ...
          <div className="flex flex-row flex-wrap gap-6 mt-4">
            {selectedPumps.map((pump) => {
              ...
              return (
                <div
                  key={pump.id}
                  className="flex flex-col items-center justify-center w-40 gap-2 px-4 py-4 text-center border rounded-xl border-blue-500 bg-white shadow-md"
                >
                  <div className="w-4 h-4 mb-2 rounded-full bg-blue-500"></div>
                  <div className="mb-2 text-gray-700">
                    {pump.pump?.title || "پمپ نامشخص"}
                  </div>
                  <div className="text-sm text-gray-600">
                    تعداد پمپ: {pump.count}
                  </div>
                  {subPumpsForThis.length > 0 && (
                    <div className="text-sm text-gray-600">
                      {subPumpsForThis.map((sub_pump) => (
                        <div key={sub_pump.id}>زیرپمپ: {sub_pump.title}</div>
                      ))}
                    </div>
                  )}
                  <button
        ...
        <Button
          className="px-10 mt-6 bg-green-600 hover:bg-green-700 text-white"
          onClick={() => {
            handleAddAndSubmit();
