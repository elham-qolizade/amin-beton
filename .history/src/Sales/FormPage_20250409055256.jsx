...
import Modal from "../pages/Modal"; // مودال را ایمپورت می‌کنیم
...
const FormPage = () => {
  ...
  const updateOrder = async (id) => {
    ...
    if (response.ok) {
      setModalMessage("اطلاعات با موفقیت به‌روزرسانی شد!"); // تنظیم پیام مودال
  ...
  const onSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      if (orderId) {
        updateOrder(orderId); // ارسال داده‌ها به سرور
      } else {
    ...
    <div className="container p-6 mt-10 rounded-lg shadow-md bg-Bokara-Grey">
      ...
      {isModalOpen && (
        <Modal
          ...
          onClose={() => setIsModalOpen(false)} // بستن مودال
        />
      )}
    </div>
  );
