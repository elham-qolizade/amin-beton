...
function SaleProject() {
  ...
  const handleTimeChange = (value) => {
    ...
    if (
      ...
      setNewOrder((prev) => ({ ...prev, delivery_time: value }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateDate(newOrder.delivery_date)) {
      toast.error("❌ تاریخ انتخابی نمی‌تواند از امروز گذشته باشد.");
      return;
    }

    if (!validateTime(newOrder.delivery_date, newOrder.delivery_time)) {
      toast.error("❌ ساعت انتخابی نمی‌تواند کمتر از زمان فعلی باشد.");
      return;
    }
  ...
  useEffect(() => {
    ...
    setNewOrder((prev) => ({ ...prev, delivery_date: formattedDate }));
  }, [selectedDay]);
