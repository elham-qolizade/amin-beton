...
const ProgressCircle = ({ status }) => {
  const [fontSize, setFontSize] = useState(
    window.innerWidth < 480 ? "0.5rem" : "0.7rem"
  );

  useEffect(() => {
    const handleResize = () => {
      setFontSize(window.innerWidth < 480 ? "0.5rem" : "0.7rem");
    };
  ...
  return (
    ...
    >
      ...
      {labels.map((label, index) => {
        ...
        } else if (label === "تایید امین بتن") {
          distanceFromCenter = 60;
        } else if (label === "پیش‌فاکتور") {
          distanceFromCenter = 57;
        } else if (label === "درخواست خرید جدید") {
          distanceFromCenter = 59;
        }
