...
const ProjectHeading = ({ title, date, subtitles, className = "" }) => {
  ...
  return (
    <div
      className={`flex items-center justify-between p-10 py-20 text-center text-white bg-center bg-no-repeat bg-cover rounded-xl backdrop-blur-md bg-black/40 shadow-xl ${className}`}
      style={{
      ...
      <div className={`container flex justify-between ${className}`}>
        ...
        >
          <div className="flex items-center">
            <h1 className="pt-4 text-lg font-bold text-right">{title}</h1>
          </div>
