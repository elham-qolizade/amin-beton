...
const HistoryProject = () => {
  ...
  // const [invoices, setInvoices] = useState([]);
  const [concreteDetails, setConcreteDetails] = useState(null);
  ...
  return (
    <div className="min-h-screen bg-b-gray">
      ...
      <div
        ...
        style={{ direction: "rtl", textAlign: "right", padding: 20 }}
      >
        <button
          className="p-2 mt-10 font-medium text-white transition-all duration-300 rounded-lg bg-School-Bus hover:bg-yellow-400 hover:text-gray-900 hover:shadow-lg hover:scale-105"
        ...
        {showModal && (
          <div
            ...
            onClick={() => setShowModal(false)} // کلیک روی بک‌دراپ مودال را می‌بندد
          >
            <div
              style={{
                backgroundColor: "#fff",
              ...
              onClick={(e) => e.stopPropagation()} // جلوگیری از بسته شدن مودال وقتی روی خود محتوا کلیک میکنیم
            >
              <button
              ...
              {orders && (
                <>
                  ...
                  <div style={{ marginBottom: 15 }}>
                    <strong>شماره سفارش: </strong> {orders.order_id}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>نام سفارش: </strong> {orders.order_name}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>زمان تحویل: </strong> {orders.delivery_datetime}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>مبلغ پرداختی: </strong> {orders.total_paid_amount}{" "}
                    تومان
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>متراژ بتن: </strong> {orders.concrete_area_size}{" "}
                    مترمربع
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>متراژ لوله‌کشی: </strong> {orders.piping_area_size}{" "}
                    مترمربع
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>ارتفاع بتن‌ریزی: </strong>{" "}
                    {orders.concrete_pouring_height} متر
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>اجرای آب‌بندی: </strong>{" "}
                    {orders.sealing_implementation}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>ماله پروانه‌ای: </strong>{" "}
                    {orders.power_trowel ? "بله" : "خیر"}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>تعداد ماله پروانه: </strong>{" "}
                    {orders.power_trowel_count}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>توضیحات: </strong>{" "}
                    {orders.additional_description || "ندارد"}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>نوع تسویه: </strong> {orders.settlement_type}
                  </div>
                  <div style={{ marginBottom: 15 }}>
                    <strong>قیمت نهایی: </strong> {orders.price} تومان
                  </div>
                  <div style={{ marginBottom: 20 }}>
                    <strong>وضعیت: </strong>{" "}
                    <span style={{ color: orders.verified ? "green" : "red" }}>
                      {orders.verified ? "تایید شده" : "تایید نشده"}
                    </span>
                  </div>
                  ...
                  <ul style={{ marginBottom: 25, paddingInlineStart: 20 }}>
                    {orders.pumps.map((pumpItem) => (
                      <li key={pumpItem.id} style={{ marginBottom: 6 }}>
                        <strong>{pumpItem.pump.title}</strong>
                        {pumpItem.sub_pump
                          ? ` - ${pumpItem.sub_pump.title}`
                          : ""}{" "}
                        : <span>{pumpItem.count} عدد</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  <ul style={{ paddingInlineStart: 20 }}>
                    {orders.vibrators.map((vibItem) => (
                      <li key={vibItem.id} style={{ marginBottom: 6 }}>
                        <strong>{vibItem.vibrator.title}</strong>
                        {vibItem.sub_vibrator
                          ? ` - ${vibItem.sub_vibrator.title}`
                          : ""}{" "}
                        : <span>{vibItem.count} عدد</span>
                      </li>
                    ))}
                  </ul>
                </>
      ...
      <div className="container flex flex-col px-4 text-right md:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 py-6 mt-4 border-b border-white md:grid-cols-2">
          ...
          {paymentSteps.length === 0 ? (
            ...
            paymentSteps.map((step) => (
              <div
                key={step.id}
                className="p-4 bg-gray-900 border border-Looking-Glass rounded-xl shadow-md flex flex-col gap-4 transition-all duration-300 hover:cursor-pointer hover:scale-[1.02]"
              >
        ...
        <div className="p-4 text-white bg-gray-800 rounded-lg">
          ...
          <div className="grid grid-cols-1 gap-6 py-6 mt-4 md:grid-cols-2">
            ...
            {/* نمایش بارنامه‌ها */}
            <div className="grid grid-cols-1 gap-2 py-6 mt-4 md:grid-cols-1">
              {Array.isArray(bills) &&
                bills.map((bill) => (
                  <div
                    key={bill.id}
                    className="p-4 bg-gray-900 border border-gray-600 rounded-xl shadow-md flex flex-col gap-4 transition-all duration-300 hover:cursor-pointer hover:scale-[1.02]"
                  >
                      ...
                      <div className="flex flex-col w-full gap-4 md:w-1/2">
                        <div>
                          <p className="mt-1 text-sm text-School-Bus">
                            وضعیت:{" "}
                            {statusEnum[bill.bill_of_lading_status] || "نامشخص"}
                          </p>
                        </div>
                    ...
                    <div className="flex justify-end md:justify-start">
                      <ButtonProject
                        className="flex justify-start w-full px-4 py-2 text-sm text-start md:w-auto md:text-base"
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="relative p-6 bg-black rounded-lg w-96">
          ...
          <button
            ...
            className="absolute text-xl font-bold text-white transition-colors top-2 left-2 hover:text-red-500"
            aria-label="بستن"
          >
            ×
          </button>
          ...
          <div className="flex gap-4 mb-4">
            ...
            <button
              ...
              className={`flex-1 px-4 py-2 text-white rounded-lg ${
                reviewChoice === "reject" ? "bg-red" : "bg-gray-600"
              }`}
            >
              رد
            </button>
