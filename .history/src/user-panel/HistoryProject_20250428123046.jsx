...
const HistoryProject = () => {
  ...
  useEffect(() => {
    const fetchLabResults = async () => {
      ...
      try {
        ...
        if (response.data) {
          setAllLabResults(response.data);
        }
      } catch (error) {
  ...
  // فیلتر و گروه‌بندی نتایج براساس دکمه انتخاب شده
  useEffect(() => {
    if (!allLabResults.length) return;

    // اول فیلتر
    const filtered = allLabResults.filter((item) => {
      ...
      return true; // اگر دکمه ای انتخاب نشده همه رو نشون بده
    });

    // بعد گروه‌بندی بر اساس دسته‌بندی
