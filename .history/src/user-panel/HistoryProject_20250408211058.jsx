...
const HistoryProject = () => {
  ...
  return (
    <div className="min-h-screen bg-b-gray">
      ...
      <ProjectHeading titleClassName="text-white font-bold" date="1402/11/10" />
      ...
      <div className="p-4 text-white bg-gray-800 rounded-lg">
        ...
        {/* //بارنامه/// */}
        <div className="p-4 text-white bg-gray-800 rounded-lg">
          <h2 className="text-lg font-bold mb-4">بارنامه‌ها</h2>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {bills.map((bill) => (
              <div
                key={bill.bill_of_lading_id}
                className="p-4 bg-Looking-Glass rounded-lg shadow-md"
              >
                <h3 className="text-xl font-semibold">{bill.driver_name}</h3>
                <p>📞 {bill.driver_mobile}</p>
                <p>
                  🚛 پلاک: {bill.plate_number_2} {bill.plate_number_alphabet}{" "}
                  {bill.plate_number_3} - {bill.plate_number_iran}
                </p>
                <p>
                  📅 {bill.bill_of_lading_date} ⏰ {bill.bill_of_lading_time}
                </p>
                <p>⚖️ وزن خالص: {bill.net_weight} کیلوگرم</p>
                <ButtonProject
                  onClick={() => {
                    setSelectedImei(bill.device.imei);
                    getLocation(bill.device.imei); // برای گرفتن موقعیت
                  }}
                >
                  📍 نمایش روی نقشه
                </ButtonProject>
            ...
            {location && (
              <div className="mt-6">
                ...
                <MapComponent
                  latitude={location.latitude} // استفاده از latitude دستی
                  longitude={location.longitude} // استفاده از longitude دستی
                  width="100%"
                  height="400px"
                />
                <ButtonProject onClick={() => getLocation(selectedImei)}>
                  🔄 بروزرسانی موقعیت
                </ButtonProject>
              </div>
            )}
