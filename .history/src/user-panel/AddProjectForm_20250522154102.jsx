...
const ProjectAPIPage = () => {
  ...
  const toIso = (dateObj) => {
    ...
    const g = (
      dateObj instanceof DateObject ? dateObj : new DateObject(dateObj)
    ).convert("gregorian");
    // خروجی با ارقام انگلیسی و دو رقمی‌کردن ماه/روز
    return g
      .format("YYYY-MM-DD")
      .replace(/[۰-۹]/g, (d) => "۰۱۲۳۴۵۶۷۸۹".indexOf(d)) // ارقام فارسی→انگلیسی
      .replace(/-(\d)(?=-|$)/g, "-0$1"); // یک‌رقمی‌ها→دو‌رقمی
  };
  ...
  const getFormattedDate = (dateObj) => {
    ...
    try {
      ...
      // ۱) تبدیل به Date جاوااسکریپت میلادی
      const jsDate = d.convert("gregorian").toDate(); // ← native Date

      // ۲) برش ISO تا YYYY-MM-DD
      return jsDate.toISOString().slice(0, 10); // مثال: "2025-05-19"
    } catch (err) {
      console.error("❌ تبدیل تاریخ شکست خورد:", err);
      return null;
    }
  };
  ...
  const validateForm = () => {
    ...
    if (values.start_date && values.end_date) {
      const start = new DateObject(values.start_date).format("YYYY-MM-DD");
      const end = new DateObject(values.end_date).format("YYYY-MM-DD");
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-900">
      <div className="w-full p-8 border rounded-lg bg-Bokara-Grey border-School-Bus">
        ...
        {responseMessage && !Object.keys(errors).length && (
          <div className="mb-4 text-center text-red">{responseMessage}</div>
        )}
          ...
          <div className="w-full md:w-1/2">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                return (
                  <div key={key} className="relative">
                    ...
                    <Input
                      ...
                      className="w-full p-2 px-10 py-2 text-white bg-gray-700 border rounded-lg bg-Bokara-Grey border-Looking-Glass placeholder:text-gray-400"
                    />
                    {errors[key] && (
              ...
              <div className="flex flex-col md:justify-center md:flex-row md:items-center md:gap-6">
                ...
                <div className="relative w-full md:w-1/2">
                  ...
                  <DatePicker
                    ...
                    inputClass="w-full m-1 z-2 text-center text-sm md:text-base text-white bg-Bokara-Grey  border border-[#FFD700] placeholder-[#aaa] p-2 rounded-lg shadow-md transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-yellow-400 hover:ring-2 hover:ring-School-Bus hover:scale-[1.03]"
                ...
                {/* تاریخ پایان */}
                <div className="relative w-full mt-4 md:w-1/2 md:mt-0">
                  <label className="block mb-1 text-Looking-Glass">
                    تاریخ پایان پروژه
                  </label>
                  <DatePicker
                    ...
                    inputClass="relative z-1  w-full m-1 text-center text-sm md:text-base text-white bg-Bokara-Grey border border-[#FFD700] placeholder-[#aaa] p-2 rounded-lg shadow-md transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-yellow-400 hover:ring-2 hover:ring-School-Bus hover:scale-[1.03]"
                    placeholder="روز / ماه / سال"
                  />
                  {errors.end_date && (
              ...
              <div className="flex justify-center">
                <ButtonProject
                  type="submit"
                  className="w-56 py-2 mb-10 font-semibold transition bg-yellow-500 rounded-lg md:w-72 text-Looking-Glass hover:bg-yellow-400"
      ...
      {showModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-Bokara-Grey  border border-School-Bus rounded-2xl shadow-2xl p-8 sm:p-10 w-[90%] max-w-sm text-center animate-fade-in">
            <div className="flex flex-col items-center gap-4">
              <FaCheckCircle className="text-5xl text-green" />
              ...
              <button
                ...
                className="px-6 py-2 mt-6 text-sm font-semibold text-white transition-all duration-200 bg-School-Bus rounded-xl hover:opacity-90"
              >
                بستن
              </button>
            </div>
