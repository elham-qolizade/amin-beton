...
const HistoryProject = () => {
  ...
  const handleUpdateLocation = (imei) => {
    console.log("Updating location for IMEI:", imei); // برای بررسی اینکه کد اجرا میشه
    getLocation(imei)
      .then((location) => {
        console.log("Location fetched:", location); // برای بررسی داده‌ای که از getLocation برگشته
        setLocations((prev) => ({
          ...prev,
          [imei]: location,
        }));
      ...
      .catch((error) => {
        console.error("Error fetching location:", error); // برای بررسی خطا
      });
  };
  ...
  const getLocation = async (imei) => {
    try {
      ...
      // به‌روزرسانی state با داده‌های دستی برای IMEI خاص
      setLocations((prevLocations) => ({
        ...prevLocations,
        [imei]: locationData,
      }));
          ...
          <div className="grid grid-cols-1 gap-6 py-6 mt-4 md:grid-cols-2">
            {bills.map((bill) => (
              ...
              >
                ...
                <ButtonProject
                  onClick={() => handleUpdateLocation(bill.device.IMEI)}
                >
