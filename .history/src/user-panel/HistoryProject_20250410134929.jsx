...
const HistoryProject = () => {
  ...
  const downloadLabCategoriesReport = () => {
    // ابتدا داده‌ها را به صورت متنی یا CSV تبدیل می‌کنیم
    let fileContent = "دسته‌بندی, عنوان, تاریخ ایجاد, لینک ویدیو\n"; // هدر CSV
    ...
    Object.keys(groupedResults).forEach((category) => {
      groupedResults[category].forEach((item) => {
        ...
        const videoLink = item.video || "ندارد";

        // اضافه کردن داده‌ها به محتوا
        fileContent += `${categoryTitle}, ${title}, ${createdAt}, ${videoLink}\n`;
      });
    });

    // ایجاد یک Blob از داده‌ها
    const blob = new Blob([fileContent], { type: "text/csv;charset=utf-8" });

    // استفاده از FileSaver.js برای دانلود فایل
    const link = document.createElement("a");
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", "lab_results_report.csv");
