...
const HistoryProject = () => {
  ...
  const [activeTab, setActiveTab] = useState(""); // مثلا "استحکام" یا "قدرت"
  const filteredResults = labResults.filter(
    (item) => item.category?.title === activeTab
  );
  ...
  const fetchLabResults = async (day = null) => {
    try {
      ...
      if (day) {
        payload.day = day; // اضافه کردن روز به درخواست اگر انتخاب شده
      }
      ...
      setLabResults(labResults); // فقط داده خام را ذخیره کن
    } catch (error) {
