...
const HistoryProject = () => {
  ...
  return (
    <div className="min-h-screen bg-b-gray">
      ...
      <div>
        <h2>سفارش شماره {order.order_id}</h2>

        <h3>مشخصات سفارش</h3>
        <p>
          <strong>تاریخ و زمان ارسال:</strong>{" "}
          {convertGeorgianDateToJalali(order.delivery_datetime.split(" ")[0])} -{" "}
          {order.delivery_datetime.split(" ")[1]}
        </p>

        <p>
          <strong>متراژ بتن:</strong> {order.concrete_area_size} متر مکعب
        </p>

        <p>
          <strong>ارتفاع پمپاژ:</strong> {order.concrete_pouring_height} متر
        </p>
        ...
        <p>
          <strong>نوع بتن:</strong>{" "}
          {concreteList.find((item) => item.id === order.concrete_type)
            ?.title ?? "نامشخص"}
        </p>

        <p>
          <strong>مقطع بتن‌ریزی:</strong>{" "}
        ...
        <p>
          <strong>اجرا و نظارت آب‌بندی:</strong>{" "}
          {order.sealing_implementation || "ندارد"}
        </p>
      </div>
