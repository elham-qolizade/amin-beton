...
const HistoryProject = () => {
  ...
  console.log("مقدار order داخل JSX:", order);
  ...
  return (
    <div className="min-h-screen bg-b-gray">
      ...
      {order ? (
        <div>
          <h2>جزئیات سفارش</h2>
          <p>تاریخ تحویل: {order.delivery_datetime}</p>
          <p>
            مبلغ پرداخت‌شده: {order.total_paid_amount.toLocaleString()} تومان
          </p>
          <p>کد سفارش: {order.order_id}</p>
          <p>مساحت بتن‌ریزی: {order.concrete_area_size} متر مربع</p>
          <p>ارتفاع بتن‌ریزی: {order.concrete_pouring_height}</p>
          <p>نوع بتن: {order.concrete_type}</p>
          <p>کلاس مقاومت بتن: {order.concrete_resistance_class}</p>
          <p>شیفت: {order.shift === 1 ? "صبح" : "عصر"}</p>
          <p>وضعیت تأیید: {order.verified ? "تأیید شده" : "در انتظار تایید"}</p>
        </div>
      ) : (
        <div className="p-4 border rounded text-red-500">
          اطلاعات سفارش موجود نیست.
        </div>
      )}
