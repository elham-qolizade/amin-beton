  ...
  return (
    <div className="min-h-screen bg-b-gray">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
        ...
        <div>
          <strong>تاریخ ایجاد:</strong> {order.created_at}
        </div>
        <div>
          <strong>متراژ بتن:</strong> {order.concrete_area_size} متر
        </div>
        <div>
          <strong>متراژ لوله‌کشی:</strong> {order.piping_area_size} متر
        </div>
        <div>
          <strong>ارتفاع بتن‌ریزی:</strong> {order.concrete_pouring_height} متر
        </div>
        <div>
          <strong>آب‌بندی:</strong>{" "}
          {order.sealing_implementation === "No" ? "خیر" : "بله"}
        </div>
        <div>
          <strong>ماله پروانه:</strong> {order.power_trowel ? "دارد" : "ندارد"}
        </div>
        <div>
          <strong>تعداد ماله:</strong> {order.power_trowel_count}
        </div>
        <div>
          <strong>شیفت:</strong> {order.shift}
        </div>
        <div>
          <strong>نحوه تسویه:</strong> {order.settlement_type}
        </div>
        <div>
          <strong>تایید شده:</strong> {order.verified ? "✅" : "❌"}
        </div>
        <div>
          <strong>وضعیت:</strong> {order.status}
        </div>
        <div>
          <strong>قیمت:</strong> {order.price.toLocaleString()} تومان
        </div>
        <div>
          <strong>نوع بتن (ID):</strong> {order.concrete_type}
        </div>
        <div>
          <strong>مقطع بتن‌ریزی (ID):</strong> {order.concrete_pouring_type}
        </div>
        <div>
          <strong>رده مقاومتی (ID):</strong> {order.concrete_resistance_class}
        </div>
      </div>
