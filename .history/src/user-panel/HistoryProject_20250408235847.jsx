...
const HistoryProject = () => {
  ...
  const fetchAndDownloadInvoice = async (orderId) => {
    ...
    try {
      ...
      // اولین پیش‌فاکتور رو استفاده می‌کنیم (میشه به دلخواه عوض کرد)
  ...
  const handleStatusChange = async (invoiceId, status) => {
    const data = {
      ...
      status: parseInt(status), // 2: تایید, 3: رد
      deny_reason: status === "3" ? denyReasons[invoiceId] : "",
    };
          ...
          <div>
            ...
            <button
              onClick={() =>
                handleStatusChange(
                  invoice.id,
                  invoiceStatuses[invoice.id], // ارسال وضعیت به API
                  invoiceStatuses[invoice.id] === "rejected"
                    ? denyReasons[invoice.id]
                    : "" // ارسال دلیل رد در صورت نیاز
                )
              }
            >
              ثبت وضعیت
