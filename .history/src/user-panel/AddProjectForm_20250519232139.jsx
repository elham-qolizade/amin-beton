...
const ProjectAPIPage = () => {
  ...
  const toIso = (dateObj) => {
    ...
    // ۱) اگر ورودی DateObject نبود اول بسازیم
    const g = (
      dateObj instanceof DateObject ? dateObj : new DateObject(dateObj)
    ).convert("gregorian");

    // ۲) «۱۴۰۴-۲-۹» → «1404-2-9» (فقط تبدیل رقم‌ها)
    let s = g
      .format("YYYY-M-D")
      .replace(/[۰-۹]/g, (d) => "0123456789"["۰۱۲۳۴۵۶۷۸۹".indexOf(d)]);

    // ۳) «1404-2-9» → «1404-02-09» (دو رقمی‌کردن)
    return s
      .split("-")
      .map((part, i) => (i ? part.padStart(2, "0") : part))
      .join("-");
  };
  useEffect(() => {
    console.log("🧪 toIso test:", toIso(new DateObject().calendar("persian")));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedStartDate = toIso(values.start_date);
    const formattedEndDate = toIso(values.end_date);
