...
const HistoryProject = () => {
  ...
  const [location, setLocation] = useState(null);
  const [locations, setLocations] = useState({}); // ذخیره موقعیت‌ها بر اساس IMEI
  const [selectedImei, setSelectedImei] = useState(null); // ذخیره IMEI انتخاب شده
  ...
  const getLocation = async (imei) => {
    try {
      ...
      setLocations((prevLocations) => ({
        ...prevLocations,
        [imei]: locationData, // ذخیره موقعیت بر اساس IMEI
      }));
    } catch (error) {
  ...
  const handleShowLocation = (imei) => {
    if (selectedImei === imei) {
      // اگر IMEI انتخاب شده باشد، موقعیت را بروزرسانی می‌کنیم
      getLocation(imei);
    } else {
      // اگر IMEI جدید باشد، آن را به عنوان انتخاب شده ذخیره می‌کنیم
  ...
  // const handleShowLocation = (imei) => {
  //   // تغییر وضعیت IMEI انتخابی
  //   if (selectedImei === imei) {
  //     // اگر IMEI انتخاب شده باشد، موقعیت را به روز می‌کنیم
  //     getLocation(imei);
  //   } else {
  //     setSelectedImei(imei);
  //     getLocation(imei);
