...
const HistoryProject = () => {
  ...
  const getInvoices = async () => {
    try {
      // درخواست برای دریافت لیست اینویس‌ها
      const invoicesResponse = await axios.get(
        "https://amin-beton-back.chbk.app/api/invoices"
      );
      const invoices = invoicesResponse.data; // فرض بر اینکه داده‌ها در اینویس‌ها قرار دارند
      ...
      // لوپ برای پردازش هر اینویس
      for (const invoice of invoices) {
        const invoiceId = invoice.id; // فرض بر اینکه id در هر اینویس موجود است

        // حالا برای هر اینویس درخواست می‌زنیم که اطلاعات فاکتور رو بگیریم
        await getInvoiceFactor(invoiceId);
      }
    } catch (error) {
  ...
  const getInvoiceFactor = async (invoiceId) => {
    try {
      // درخواست برای گرفتن فاکتور مربوط به هر اینویس
      const response = await axios.post(
        "https://amin-beton-back.chbk.app/api/orders/get-order-factor/",
        {
          order_id: invoiceId, // ارسال شناسه اینویس برای دریافت اطلاعات فاکتور
        },
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const factor = response.data; // اطلاعات فاکتور دریافتی

      if (!factor) {
        console.log(`⛔ فاکتوری برای اینویس ${invoiceId} پیدا نشد.`);
        return;
      }

      console.log(`💰 قیمت فاکتور اینویس ${invoiceId}:`, factor.price);
      console.log(`🧾 فایل فاکتور اینویس ${invoiceId}:`, factor.invoice_file);

      // انجام عملیات مورد نظر با داده‌های فاکتور، مثلا تغییر وضعیت یا هر کار دیگری
    } catch (error) {
      console.error(`❌ خطا در دریافت فاکتور اینویس ${invoiceId}:`, error);
      alert(`⚠️ خطا در دریافت فاکتور اینویس ${invoiceId}.`);
    }
  };

  // فراخوانی تابع اصلی برای گرفتن اینویس‌ها
  getInvoices();
