...
const HistoryProject = () => {
  ...
  const fetchAndDownloadInvoice = async (
    ...
    try {
      ...
      // اولین پیش‌فاکتور رو انتخاب می‌کنیم
      ...
      // اگر وضعیت فاکتور تغییر پیدا کرده
      if (status !== null) {
        const data = {
          invoice_id: invoice.id,
          status: parseInt(status), // 1: تایید، 2: رد
          deny_reason: status === "2" ? denyReason : "",
        };

        // تغییر وضعیت فاکتور
        await axios.post(
          "https://amin-beton-back.chbk.app/api/invoices/change-invoice-status/",
          data,
          {
            headers: {
              "Content-Type": "application/json",
      ...
      const denyReasonText =
        invoice.status === 2
          ? `\n🛑 دلیل رد: ${invoice.deny_reason || "ندارد"}`
          : "";
      ...
      link.setAttribute(
        "download",
        `invoice_report_${invoice.id}_${orderId}.txt`
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);

      alert("✅ فایل پیش‌فاکتور با موفقیت دانلود شد.");
    } catch (error) {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        "https://amin-beton-back.chbk.app/api/lab-result-items/", // فرض بر این است که این آدرس داده‌ها رو می‌گیره
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  // برای فراخوانی در هنگام بارگذاری صفحه
  useEffect(() => {
    getCategories();
    getData();
  }, [token]);

  // گروهبندی داده‌ها بر اساس category و ترتیب نزولی بر اساس created_at
  ...
  // دانلود فایل گزارش دسته‌بندی‌ها
  ...
  return (
    <div className="min-h-screen bg-b-gray">
      ...
      <div className="flex flex-row-reverse  w-full gap-10  justify-between  container ">
        ...
        <div className="flex flex-col text-right ">
          ...
          <div className="flex gap-2 flex-col">
            ...
            < className="border-b py-10 border-white">
           {/* نمایش گروه‌بندی‌های داده‌ها */}
      {groupedData.length > 0 ? (
        groupedData.map((group) => (
          <div key={group.category} className="mb-6">
            <h2 className="text-xl font-bold mb-2">{group.category}</h2>
            {group.items.map((item) => (
              <div key={item.id} className="border p-2 rounded mb-1">
                📹 {item.title} <br />
      ...
      {/* دکمه دانلود گزارش */}
      <button onClick={downloadLabCategoriesReport} className="mt-4 p-2 bg-blue-500 text-white rounded">
        دانلود گزارش دسته‌بندی‌ها
      </button>
