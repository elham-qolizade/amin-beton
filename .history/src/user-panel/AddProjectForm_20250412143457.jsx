...
// import DateObject from "react-date-object"; // اضافه کن بالا اگر نیست
...
const ProjectAPIPage = () => {
  ...
  const handleLocationSelect = ([lat, lng]) => {
    ...
    console.log("📍 مختصات انتخاب‌شده از نقشه:", coords);
  ...
  // import DateObject from "react-date-object"; // اطمینان از ایمپورت

  const getFormattedDate = (dateObj) => {
    try {
      const date = new DateObject(dateObj); // تبدیل به DateObject
      return date.toGregorian().format("YYYY-MM-DD");
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("🚨 start_date value before submit:", values.start_date);
                  ...
                  {key.includes("date") ? (
                    <div className="relative w-full p-1 px-4 py-2 pl-4 text-black bg-gray-700 border rounded ltr-input bg-Bokara-Grey border-l-Looking-Glass focus:outline-none focus:ring-1 focus:ring-School-Bus border-Looking-Glass">
                      <DatePicker
                        ...
                        value={values[key]}
                        onChange={(date) => {
                          const realDate = new DateObject(date); // مطمئن شو که این واقعاً DateObject باشه
