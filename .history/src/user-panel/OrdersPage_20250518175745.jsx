...
export default function OrdersPage() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      setIsPaymentsModalOpen(true); // باز کردن فقط مودال پرداخت
      // باز کردن مودال گردش حساب
    } catch (error) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-Bokara-Grey">
      ...
      <div className="flex justify-center">
        {projectInfo && (
          <div className="w-full max-w-5xl px-4 py-6 mt-6 text-white bg-Bokara-Grey rounded-lg shadow-lg">
            <h3 className="mb-4 text-lg font-bold border-b border-gray-600 pb-2">
              خلاصه مالی پروژه
            </h3>
            {/* <p className="text-sm sm:text-base">
              <span className="font-semibold">مجموع وزن کل خریدها:</span>{" "}
              {projectInfo.total_weight?.toLocaleString()} کیلوگرم
            </p> */}
            ...
            <button
              ...
              className="px-4 py-2 mt-4 text-black bg-white rounded-md hover:bg-School-Bus"
            >
              مشاهده گردش حساب
            </button>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex flex-col flex-1 gap-2 pb-5 text-start md:text-right">
                    ...
                    {/* نمایش مبلغ پرداختی اگر وجود دارد */}
                    {/* <p className="text-sm">
                      مبلغ:{" "}
                      {payment
      ...
      {isPaymentsModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="w-full max-w-2xl p-6 bg-black overflow-y-auto text-white bg-gray-800 rounded-lg max-h-[80vh]">
            <h2 className="mb-4 text-xl font-bold text-center border-b pb-2">
              گردش حساب پروژه
            </h2>
            {payments.length === 0 ? (
              ...
              <div className="h-80 overflow-y-auto border border-gray-300 rounded-md p-2">
                <ul className="flex flex-col gap-3">
                  {payments.map((pay) => (
                    <li
                      key={pay.id}
                      className="p-3 bg-gray-700 rounded-md shadow hover:bg-gray-600 text-white"
                    >
            ...
            <div className="flex justify-end mt-4">
              <button
                ...
                className="px-4 py-2 text-black bg-white rounded hover:bg-School-Bus"
              >
                بستن
              </button>
            </div>
