...
const HistoryProject = () => {
  ...
  const getOrderIdFromURL = () => {
    const urlParams = new URLSearchParams(window.location.search); // گرفتن پارامترهای URL
    const orderId = urlParams.get("id"); // گرفتن مقدار 'id' از URL (مثلاً '123')
  ...
  const getOrderIdFromURL = () => {
    const urlParams = new URLSearchParams(window.location.search); // گرفتن پارامترهای URL
    return urlParams.get("id"); // پارامتر 'id' رو می‌گیریم (مثلاً '123')
  };

  const handleStatusChange = async (invoiceId, status, denyReason = "") => {
    const orderId = getOrderIdFromURL(); // گرفتن orderId از URL
    console.log("Order ID:", orderId);

    // بررسی می‌کنیم که آیا orderId معتبر است یا نه
    if (!orderId || isNaN(orderId)) {
      alert("⚠️ لطفاً یک شناسه سفارش معتبر وارد کنید.");
      return;
    }
    ...
    try {
      // ابتدا درخواست برای دریافت فاکتورها
      const response = await axios.post(
        "https://amin-beton-back.chbk.app/api/orders/get-order-invoices/",
        { order_id: parseInt(orderId) },
        {
      ...
      // فرض بر این است که اولین فاکتور مورد استفاده قرار می‌گیرد
      const invoice = invoices[0];

      // چک می‌کنیم که فایل فاکتور وجود دارد یا نه
      if (!invoice.invoice_file) {
        alert("⚠️ پیش‌فاکتور یافت شد اما فایل ندارد.");
        return;
      }

      // حالا وضعیت فاکتور را تغییر می‌دهیم
      await axios.post(
      ...
      window.location.reload(); // رفرش صفحه برای دریافت وضعیت جدید فاکتورها
    } catch (error) {
      console.error("❌ خطا در دریافت یا تغییر وضعیت فاکتورها:", error);
      alert("⚠️ مشکلی در دریافت یا تغییر وضعیت پیش آمد.");
    }
  };
