...
const HistoryProject = () => {
  ...
  const fetchOrderPaymentStep = async (paymentStepId) => {
    if (!paymentStepId || isNaN(paymentStepId)) {
    ...
    try {
      setLoading(true);
      const response = await axios.get(
        `https://amin-beton-back.chbk.app/api/order-payment-steps/${paymentStepId}/`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const paymentStep = response.data;

      if (!paymentStep || !paymentStep.id) {
        console.warn("⛔ پرداختی یافت نشد.");
        setPaymentStep(null);
        return;
      }

      console.log("✅ Payment Step data:", paymentStep);
      setPaymentStep(paymentStep); // assuming you're storing a single object
    } catch (error) {
      console.error("❌ خطا در دریافت مرحله پرداخت:", error);
      setError("خطا در دریافت مرحله پرداخت");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-b-gray">
      ...
      <div className="py-6 border-b border-white md:py-10">
        ...
        {paymentSteps.length === 0 ? (
          <p className="text-center text-red">هیچ پرداختی ثبت نشده است.</p>
        ) : (
          <div className="flex flex-col gap-6">
            {paymentSteps.map((step) => (
              <div
                key={step.id}
                className="flex flex-col gap-2 p-4 text-white bg-gray-700 rounded-lg"
              >
                <div className="flex justify-between">
                  <p>
                    عنوان: <span className="font-bold">{step.title}</span>
                  </p>
                  <p>
                    مبلغ:{" "}
                ...
                {step.note && <p>یادداشت: {step.note}</p>}
                {step.attachment && (
                  <a
