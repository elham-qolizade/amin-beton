          ...
          {projects.length === 0 ? (
            ...
            projects.map((p) => (
              <div
                ...
                className="relative flex h-56 w-full cursor-pointer flex-col justify-between rounded-2xl bg-black p-4 shadow-xl transition hover:scale-[1.02] hover:shadow-2xl md:h-64"
              >
                {/* سرآیند */}
                <h3 className="text-base font-bold text-School-Bus">
                  {p.title}
                </h3>

                {/* بدنهٔ اطلاعات */}
                <div className="mt-2 space-y-1 text-sm leading-5 text-gray-200">
                  <p>
                ...
                <span
                  className={
                    p.status === 1
                      ? "self-start rounded-full bg-emerald-600/20 px-3 py-1 text-xs font-medium text-emerald-400"
                      : p.status === 2
                      ? "self-start rounded-full bg-red-600/20 px-3 py-1 text-xs font-medium text-red-400"
                      : "self-start rounded-full bg-yellow-500/20 px-3 py-1 text-xs font-medium text-yellow-400"
                  }
                >
                  {p.status === 1
                    ? "در حال انجام"
