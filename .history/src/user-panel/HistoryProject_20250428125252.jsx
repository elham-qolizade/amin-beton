...
const HistoryProject = () => {
  ...
  const categories = [
    { id: 1, title: "استحکام", days: [3, 7, 14] },
    { id: 2, title: "قدرت", days: [7, 14] },
    { id: 3, title: "مقاومت", days: [3] },
  ];
      ...
      <div className="container flex flex-col px-4 text-right md:px-8">
        ...
        <div className="py-6 border-b border-white md:py-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* دکمه‌های انتخاب آزمایش بر اساس روز */}
            <div className="flex flex-wrap gap-4 mt-4">
              {categories
                .filter((category) => category.days && category.days.length > 0) // فقط دسته‌بندی‌هایی که روز دارن رو فیلتر می‌کنیم
                .map(
                  (category) =>
                    selectedCategory === category.title && // فقط برای دسته‌بندی انتخاب‌شده دکمه‌ها رو نمایش می‌دهیم
                    category.days.map((day) => (
                      <button
                        key={day}
                        onClick={() => handleClick(day)} // تابعی که برای آزمایش‌های ۳، ۷، ۱۴ روزه زدی
