...
const HistoryProject = () => {
  ...
  const handleStatusChange = async (orderId, status, denyReason = "") => {
    // ابتدا درخواست get-order-factor برای دریافت فاکتور
    try {
      const response = await axios.post(
        "https://amin-beton-back.chbk.app/api/orders/get-order-factor/",
        { order_id: parseInt(orderId) },
      ...
      // فرض بر اینکه فاکتور در response.data قرار دارد و id فاکتور را از آن استخراج می‌کنیم
      const invoiceId = response.data?.invoice_id; // یا بسته به ساختار پاسخ، این مقدار را اصلاح کنید

      if (!invoiceId) {
        alert("⛔ فاکتوری برای این سفارش پیدا نشد.");
        return;
      }

      const data = {
        invoice_id: invoiceId, // استفاده از invoiceId که از پاسخ گرفته شده است
