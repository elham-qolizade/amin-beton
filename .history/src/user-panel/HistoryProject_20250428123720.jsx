...
const HistoryProject = () => {
  ...
  const getCategories = async () => {
    try {
      const token = localStorage.getItem("accessToken"); // این خط اضافه شد
      if (!token) {
      ...
      if (response.data) {
        ...
        if (response.data.length > 0) {
          setSelectedCategory(response.data[0].title); // انتخاب پیش‌فرض اولین دسته
        }
      }
    } catch (error) {
  ...
  // گرفتن نتایج
  const fetchLabResultsByDay = async (day) => {
    try {
      ...
      const response = await axios.post(
        ...
        {
          order_id: id,
          day: day, // این خط مهمه 👈 داده رو با فیلتر روز میفرستیم
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.error("Error fetching lab results by day:", error);
    }
  };
  ...
  const handleClick = (day) => {
    setSelectedButton(day); // انتخاب ظاهری دکمه
    fetchLabResultsByDay(day); // بیار نتایج واقعی بر اساس روز
  };
