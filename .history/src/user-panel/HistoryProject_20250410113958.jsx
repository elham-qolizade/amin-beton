...
const HistoryProject = () => {
  ...
  const handleShowLocation = async (billId, imei) => {
    setSelectedBillId(billId);
    // Fetch location based on imei
    const fetchedLocation = await getLocationByIMEI(imei); // این تابع رو شما داری
      ...
      <div className="p-4 text-white bg-gray-800 rounded-lg">
        ...
        <div className="grid grid-cols-1 gap-6 py-6 mt-4 md:grid-cols-2">
          ...
          {bills.map((bill) => (
            <div
              key={bill.id}
              className="p-4 mb-4 border rounded-lg bg-white/10"
            >
              ...
              <ButtonProject
                onClick={() => handleShowLocation(bill.id, bill.device.IMEI)}
              >
              ...
              {selectedBillId === bill.id && location && (
                <div className="mt-6">
