  ...
  useEffect(() => {
    const fetchLabResults = async () => {
      try {
        ...
        const token = localStorage.getItem("accessToken"); // یا از روش دیگه‌ای که تو ذخیره می‌کنی

        // بررسی اینکه آیا Access Token موجود است یا نه
        if (!token) {
          // باید از token استفاده کنی، نه accessToken
        ...
        // ارسال درخواست POST با Access Token در هدر Authorization
        const response = await axios.post(
          "https://amin-beton-back.chbk.app/api/lab-result/get-order-lab-results/",
          {
            order_id: id, // ID از useParams
          },
          {
            headers: {
              Authorization: `Bearer ${token}`, // ارسال Access Token در هدر
              "Content-Type": "application/json",
            },
          }
        );

        // اگر داده‌ای دریافت شد، گروه‌بندی بر اساس دسته‌بندی‌ها انجام بده
        if (response.data) {
      ...
      <div className="container flex flex-col px-4 text-right md:px-8">
        ...
        {loading ? (
          ...
          Object.entries(groupedResults).map(([category, items]) => (
            <div key={category} className="p-4 mb-4 border rounded-lg shadow">
