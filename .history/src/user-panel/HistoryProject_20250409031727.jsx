...
const HistoryProject = () => {
  ...
  useEffect(() => {
    // فرض می‌کنیم که داده‌ها رو از API می‌گیریم
    fetchData().then((data) => {
      console.log("Invoices data:", data); // اینجا داده‌ها رو چاپ می‌کنیم
      setInvoices(data);
    });
  }, []);
            ...
            <div className="py-10 border-b border-white bg-School-Bus">
              ...
              {invoices.map((invoice) => (
                ...
                >
                  ...
                  <p>
                    <strong>قیمت:</strong> {invoice.price.toLocaleString()}{" "}
                    تومان
                  </p>
                  <p>
                    <strong>وضعیت:</strong>{" "}
                    {statusMap[invoice.status] || "وضعیت نامشخص"}
                  </p>
                  <a
