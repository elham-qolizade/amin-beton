...
const HistoryProject = () => {
  ...
  const downloadLabResultsPDF = () => {
    ...
    // عنوان PDF
    doc.setFontSize(18);
    doc.text(`📚 Lab Results Report (Order ID: ${id})`, 14, 20);
    doc.setFontSize(12);
    doc.text("=========================", 14, 30);

    // برای هر دسته‌بندی نتایج آزمایشات
    Object.keys(groupedResults).forEach((category, index) => {
      doc.text(`Category: ${category}`, 14, 40 + index * 10);

      // اضافه کردن آزمایشات هر دسته
      groupedResults[category].forEach((test, testIndex) => {
        const yPosition = 50 + (index + testIndex) * 20;

        doc.text(`Title: ${test.title}`, 14, yPosition);
        doc.text(
          `Created At: ${moment(test.created_at).format(
        ...
        if (test.video) {
          doc.text(`Video Link: ${test.video}`, 14, yPosition + 10);
        } else {
          doc.text("Video Link: No video available", 14, yPosition + 10);
        }

        doc.text("-------------------------", 14, yPosition + 15);
      });
      doc.text("=========================", 14, 60 + index * 20);
    });

    // دانلود فایل PDF
    doc.save(`lab_results_report_${id}.pdf`);
  };
