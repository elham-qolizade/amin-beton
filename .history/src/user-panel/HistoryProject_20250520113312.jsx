...
const HistoryProject = () => {
  ...
  return (
    <div className="min-h-screen bg-b-gray">
      ...
      <div className="p-4">
        <h2 className="text-xl font-bold mb-4">سفارشات پروژه {id}</h2>

        {loading && <p>در حال بارگذاری...</p>}
        {error && <p className="text-red-600">❌ {error}</p>}
        {!loading && orders.length === 0 && (
          <p>هیچ سفارشی برای این پروژه ثبت نشده.</p>
        )}

        <ul className="mt-4 space-y-2">
          {orders.map((order) => (
            <li key={order.id} className="border p-2 rounded shadow-sm">
              <p>
              ...
              <p>
                <strong>تأیید شده:</strong> {order.verified ? "بله" : "خیر"}
              </p>
            </li>
          ))}
        </ul>
