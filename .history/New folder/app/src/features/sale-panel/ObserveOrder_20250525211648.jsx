...
<InfoItem>
  ...
  {isEditMode ? (
    ...
    <span>
      {concreteList.find((c) => c.id === order.concrete_type)?.title || "نامشخص"}
    </span>
  )}
</InfoItem>
  ...
  {editableOrder.pumps.length === 0 ? (
    ...
    editableOrder.pumps.map((pump, index) => {
      ...
      const handlePumpChange = (e) => {
        const selectedId = parseInt(e.target.value, 10);
        const selectedPumpObj = pumpList.find((p) => p.id === selectedId) || null;
      ...
      const handleSubPumpChange = (e) => {
        const selectedId = parseInt(e.target.value, 10);
        const selectedSubPump = subPumpList.find((sp) => sp.id === selectedId) || null;

        const updated = [...editableOrder.pumps];
        updated[index] = { ...updated[index], sub_pump: selectedSubPump };
        setEditableOrder((prev) => ({ ...prev, pumps: updated }));
      };
      ...
      return (
        <Pump key={index}>
          ...
          <InfoItem>
            ...
            {isEditMode ? (
              isLoadingSubPumpList ? (
                ...
                <select value={pump.sub_pump?.id || ""} onChange={handleSubPumpChange}>
          ...
          <InfoItem>
            تعداد:
            {isEditMode ? (
              <input type="number" value={pump.count} onChange={handleCountChange} />
            ) : (
...
<Pumps>
  {(editableOrder.vibrators?.length ?? 0) === 0 ? (
    ...
    editableOrder.vibrators.map((vib, index) => {
      ...
      const handleVibChange = (e) => {
        const selectedId = parseInt(e.target.value, 10);
        const selectedObj = vibratorList.find((v) => v.id === selectedId) || null;

        const updated = [...editableOrder.vibrators];
        updated[index] = { ...updated[index], vibrator: selectedObj, sub_vibrator: null };
      ...
      const handleSubVibChange = (e) => {
        const selectedId = parseInt(e.target.value, 10);
        const selectedSubVib = subVibratorList.find((s) => s.id === selectedId) || null;

        const updated = [...editableOrder.vibrators];
        updated[index] = { ...updated[index], sub_vibrator: selectedSubVib };
        setEditableOrder((prev) => ({ ...prev, vibrators: updated }));
      };
      ...
      return (
        <Pump key={index}>
          ...
          <InfoItem>
            ...
            {isEditMode ? (
              isLoadingSubVibratorList ? (
                ...
                <select value={vib.sub_vibrator?.id || ""} onChange={handleSubVibChange}>
                  <option value="">ندارد</option>
          ...
          <InfoItem>
            تعداد:
            {isEditMode ? (
              <input type="number" value={vib.count} onChange={handleVibCountChange} />
            ) : (
              <span>{vib.count}</span>
            )}
          </InfoItem>
  ...
  {isEditMode && (
    <button
      ...
      onClick={() =>
        setEditableOrder((prev) => ({
          ...prev,
          vibrators: [...(prev.vibrators || []), { vibrator: null, sub_vibrator: null, count: 1 }],
        }))
      }
    >
  ...
  {isEditMode ? (
    <input
      ...
      onChange={(e) => handleChange("sealing_implementation", e.target.value)}
    />
  ) : (
