...
function ObserveOrder({ order }) {
  ...
  useEffect(() => {
    if (
      !order ||
      !Array.isArray(order.vibrators) ||
      !Array.isArray(vibratorList) ||
      vibratorList.length === 0
    ) {
      return;
    }

    const enrichedVibrators = order.vibrators.map((v) => {
      const vibratorObj = vibratorList.find((vb) => vb.id === v.vibrator);
      const subVibratorObj = vibratorObj?.sub_vibrators?.find(
        (sv) => sv.id === v.sub_vibrator
      );

      return {
        vibrator: vibratorObj || null,
        sub_vibrator: subVibratorObj || null,
        count: v.count,
      };
    });
                    ...
                    {isEditMode ? (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (selected) {
                            updated[index].vibrator = selected; // کل شیء همراه sub_vibrators
                      ...
                      >
                        ...
                        <span>
                          {vibratorList
                            .find((v) => v.id === vibrator.vibrator)
                            ?.sub_vibrators?.find(
                              (sv) => sv.id === vibrator.sub_vibrator
                            )?.title || "ندارد"}
                        </span>
                      </select>
                    ) : (
                  ...
                  <InfoItem>
                    ...
                    {isEditMode ? (
                      <select
                        ...
                        onChange={(e) => {
                          const updated = [...editableOrder.vibrators];
                          const parent = updated[index].vibrator;

                          const selectedSub = parent?.sub_vibrators?.find(
                      ...
                      >
                        <option value="">ندارد</option>
                        {(vibrator.vibrator?.sub_vibrators || []).map((sv) => (
