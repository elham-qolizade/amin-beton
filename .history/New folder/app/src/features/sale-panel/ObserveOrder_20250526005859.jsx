                  ...
                  <InfoItem>
                    ...
                    {isEditMode ? (
                      <select
                        value={pump.pump?.id || ""}
                        onChange={async (e) => {
                          const selectedPumpId = parseInt(e.target.value);
                          const selectedPump =
                            pumpList.find((p) => p.id === selectedPumpId) ||
                            null;
                          const subPumpOptions = selectedPumpId
                            ? await fetchSubPumps(selectedPumpId)
                            : [];
                          ...
                          const updatedPumps = editableOrder.pumps.map((p, i) =>
                            i === index
                              ? {
                                  ...
                                  pump: selectedPump,
                                  sub_pump: null,
                                  sub_pump_options: subPumpOptions, // ✅ اضافه کن
                                }
                              : p
                          );
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            const selectedSubPump =
                              (pump.sub_pump_options || []).find(
                                (sp) => sp.id === parseInt(e.target.value)
                              ) || null;

                            const updatedPumps = editableOrder.pumps.map(
                              (p, i) =>
                                i === index
