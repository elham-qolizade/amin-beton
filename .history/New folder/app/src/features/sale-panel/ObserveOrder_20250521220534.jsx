...
function ObserveOrder({ order }) {
  ...
  const handleSave = async () => {
    await axios.patch(
      "https://amin-beton-back.chbk.app/order-management/update-pump-order/",
      {
    ...
    await axios.patch("/order-management/update-vibrator-order/", {
      order_id: editableOrder.id,
        ...
        <Pumps>
          {editableOrder.pumps.length === 0 ? (
            ...
            editableOrder.pumps.map((pump, index) => {
              const subPumpOptions = pump.pump?.sub_pumps || [];
              return (
                <Pump key={index}>
                    ...
                    {isEditMode ? (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          updated[index].pump = pumpList.find(
                  ...
                  <InfoItem>
                    <button
                      ...
                      onClick={() => {
                        const updated = [...editableOrder.pumps];
                        updated.splice(index, 1); // حذف این پمپ
                        setEditableOrder((prev) => ({
                          ...prev,
                          pumps: updated,
                        }));
            ...
            <button
              ...
              onClick={() =>
                setEditableOrder((prev) => ({
                  ...
                  pumps: [
                    ...prev.pumps,
                    { pump: null, sub_pump: null, count: 1 }, // مقادیر پیش‌فرض
                  ],
                }))
              }
            >
