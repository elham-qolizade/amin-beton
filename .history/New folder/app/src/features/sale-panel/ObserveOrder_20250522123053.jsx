...
function ObserveOrder({ order }) {
  ...
  const saveVibrators = async (headers) => {
    const cleanVibrators = editableOrder.vibrators.map((v) => {
      return {
        vibrator:
          v.vibrator && typeof v.vibrator === "object"
            ? v.vibrator.id
            : parseInt(v.vibrator),
        sub_vibrator:
          v.sub_vibrator && typeof v.sub_vibrator === "object"
            ? v.sub_vibrator.id
            : v.sub_vibrator !== null
            ? parseInt(v.sub_vibrator)
            : null,
    ...
    if (!res.ok) {
      ...
      console.error("خطا از سمت سرور:", errorText);
  ...
  const savePumps = async (headers) => {
    const body = {
      ...
      pumps: editableOrder.pumps.map((p) => {
        const selectedPump = pumpList.find((pump) => pump.id === p.pump?.id);
        const requiresSubPump = selectedPump?.sub_pumps?.length > 0;

        // اگر زیرمجموعه نیاز هست ولی انتخاب نشده، throw نکن، بذار API خطا بده
        return {
          pump: p.pump?.id || null,
          sub_pump: requiresSubPump ? p.sub_pump?.id : null,
          count: parseInt(p.count),
        };
      }),
    };

    console.log("BODY که برای پمپ ارسال میشه:", body);
    ...
    if (!res.ok) {
      ...
      console.error("خطای پمپ:", errorData);
        ...
        <Vibrators>
          {editableOrder.vibrators.length === 0 ? (
            ...
            editableOrder.vibrators.map((vibrator, index) => {
              const selectedVibrator = vibratorList.find(
