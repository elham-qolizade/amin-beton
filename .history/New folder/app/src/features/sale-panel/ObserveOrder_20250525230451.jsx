...
function ObserveOrder({ order, onUpdate }) {
  ...
  const handleSave = async () => {
    try {
      ...
      // PATCH سفارش با داده‌های ویرایش شده
      ...
      // دریافت داده کامل سفارش بعد از PATCH (اگر API PATCH داده کامل نمی‌دهد)
      ...
      if (!refreshedOrderRes.ok)
        throw new Error("دریافت سفارش به‌روز با خطا مواجه شد");

      const refreshedOrder = await refreshedOrderRes.json();

      // ذخیره داده‌های وابسته (مثل پمپ‌ها و ویبراتورها)
      await savePumps(headers);
      await saveVibrators(headers);

      alert("سفارش با موفقیت ذخیره شد");

      // ارسال داده به‌روز به کامپوننت والد برای آپدیت کش react-query و UI بلافاصله
      if (typeof onUpdateOrder === "function") {
        onUpdateOrder(refreshedOrder);
      }

      // آپدیت داخلی state کامپوننت
