...
function ObserveOrder({ order }) {
  ...
  const handleVibratorChange = (index, key, value) => {
    setVibrators((prev) => {
      ...
      updated[index] = {
        ...updated[index],
        [key]: value,
        ...(key === "vibrator" ? { sub_vibrator: "" } : {}), // پاک کردن زیرمجموعه
      };
      return updated;
    });
  };
  ...
  const vibratorList = useMemo(() => {
    ...
    return parents.map((parent) => ({
      ...parent,
      sub_vibrators: flatList.filter(
        (v) => v.parent && Number(v.parent) === Number(parent.id)
      ),
    }));
  }, [flatList]);
  useEffect(() => {
    if (editableOrder?.vibrators && vibratorList.length > 0) {
      const enrichedVibrators = editableOrder.vibrators.map((v) => {
        const mainVibrator =
          vibratorList.find((vib) => vib.id === v.vibrator) || null;
        const subVibrator =
          mainVibrator?.sub_vibrators.find((sv) => sv.id === v.sub_vibrator) ||
          null;
        return {
          vibrator: mainVibrator,
          sub_vibrator: subVibrator,
          count: v.count,
        };
      });
      setEditableOrder((prev) => ({ ...prev, vibrators: enrichedVibrators }));
    }
  }, [editableOrder?.vibrators, vibratorList]);

  useEffect(() => {
    if (!order?.vibrators || flatList.length === 0) return;

    const vibratorsWithObjects = order.vibrators.map((v) => {
      const vibratorObj =
        flatList.find((item) => item.id === v.vibrator) || null;
      const subVibratorObj =
        flatList.find((item) => item.id === v.sub_vibrator) || null;

      return {
        vibrator: vibratorObj,
        sub_vibrator: subVibratorObj,
        count: v.count,
      };
    });
    ...
    setEditableOrder((prev) => ({
      ...prev,
      vibrators: vibratorsWithObjects,
    }));
  }, [order, flatList]);
                    ...
                    {isEditMode ? (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const selectedSub = vibratorList.find(
                            (v) => v.id === subId
                          );
                      ...
                      >
                        <option value="">ندارد</option>
                        {vibratorList
                          .filter((v) => v.parent === vibrator.vibrator?.id)
                          .map((sv) => (
