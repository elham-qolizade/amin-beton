        ...
        <Vibrators>
          {editableOrder.vibrators.length === 0 ? (
            ...
            editableOrder.vibrators.map((vibrator, index) => {
              // پیدا کردن آبجکت ویبراتور اصلی و زیرمجموعه بر اساس آیدی
              const mainVibrator = vibratorList.find(
                (v) => v.id === vibrator.vibrator
              );
              const subVibrator = vibratorList.find(
                (v) => v.id === vibrator.sub_vibrator
              );
                    ...
                    {isEditMode ? (
                      ...
                      >
                        ...
                        {vibratorList
                          .filter(
                            (v) =>
                              Number(v.parent) === Number(vibrator.vibrator)
                          )
                          .map((sv) => (
                      ...
                      <span>{subVibrator?.title || "ندارد"}</span>
                    )}
                  </InfoItem>
