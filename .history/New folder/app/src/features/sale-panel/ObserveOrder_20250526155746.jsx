...
function ObserveOrder({ order, onUpdate }) {
  ...
  export function formatDateTime(inputDate, format = "YYYY/MM/DD HH:mm") {
    if (!inputDate) return "نامشخص";
    try {
      const date = dayjs(inputDate);
      if (!date.isValid()) return "نامعتبر";
        ...
        <InfoItem className="half-row-2">
          ...
          {isEditMode ? (
            <DatePicker
              ...
              onChange={(value) =>
                handleChange("delivery_datetime", value?.toDate?.() || "")
              }
            ...
            <span>
              {formatDate(editableOrder.delivery_datetime)} -{" "}
