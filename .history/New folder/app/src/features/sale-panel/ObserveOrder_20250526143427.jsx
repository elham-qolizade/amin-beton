...
function ObserveOrder({ order, onUpdate }) {
  ...
  const validateForm = () => {
    ...
    editableOrder.pumps.forEach((pump, index) => {
      const pumpError = {};
      if (!pump.pump?.id) {
        pumpError.pump = "لطفاً نوع پمپ را انتخاب کنید.";
      }
      const subPumpList = subPumpLists[pump.pump?.id];
      if (subPumpList?.length && !pump.sub_pump?.id) {
        pumpError.sub_pump = "لطفاً یک زیرپمپ انتخاب کنید.";
      }
      errors.pumps[index] = pumpError;
    });
                    ...
                    {isEditMode ? (
                      !subPumpLists[currentPumpId] ? (
                        ...
                        <>
                          ...
                          {pumpErrors[`pump_${index}`]?.sub_pump && (
                            <ErrorMessage>
                              {pumpErrors[`pump_${index}`].sub_pump}
                            </ErrorMessage>
                          )}
