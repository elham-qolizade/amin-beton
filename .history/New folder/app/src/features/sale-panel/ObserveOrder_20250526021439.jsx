...
function ObserveOrder({ order, onUpdate }) {
  ...
  const validateForm = () => {
    ...
    if (vib.vibrator) {
      const subListForThisVibrator = subVibratorList.filter(
        (s) => s.parentId === vib.vibrator.id
      );
      console.log("filtered subList", subListForThisVibrator);

      if (subListForThisVibrator.length > 0 && (!vib.sub_vibrator || !vib.sub_vibrator.id)) {
        errors[`sub_vibrator_${index}`] = `زیرمجموعه ویبراتور شماره ${index + 1} باید انتخاب شود.`;

        if (!subVibratorErrorShown) {
          alert(`لطفا زیرمجموعه ویبراتورها را کامل انتخاب کنید.`);
          subVibratorErrorShown = true;
        }
      }
    }
  });
  ...
  useEffect(() => {
    ...
    (editableOrder.vibrators || []).forEach((vib, index) => {
      ...
      if (vib.vibrator) {
        // فیلتر کردن زیرمجموعه‌های مربوط به ویبراتور فعلی
        const subListForThisVibrator = subVibratorList.filter(
          (s) => s.parentId === vib.vibrator.id
        );

        // اگر زیرمجموعه برای این ویبراتور وجود دارد ولی انتخاب نشده است
        if (
          subListForThisVibrator.length > 0 &&
          (!vib.sub_vibrator || !vib.sub_vibrator.id)
        ) {
          errors[`sub_vibrator_${index}`] = `زیرمجموعه ویبراتور شماره ${
            index + 1
          } باید انتخاب شود.`;
          alert(`زیرمجموعه ویبراتور شماره ${index + 1} انتخاب نشده است!`);
        }
      }
    });

    setErrors(newErrors);
  }, [editableOrder.vibrators]);
                      ...
                      isLoadingSubVibratorList ? (
                        ...
                        <>
                          ...
                          >
                            <option value="">ندارد</option>
                            {subVibratorList
                              .filter((s) => s.parentId === vib.vibrator?.id) // فیلتر برای زیرمجموعه‌های ویبراتور فعلی
                              .map((s) => (
