...
function ObserveOrder({ order }) {
  ...
  const handleChange = (newValue) => {
    if (!newValue) return;
    const formatted = newValue.format("YYYY-MM-DD HH:mm:ss"); // میلادی ذخیره شود
    setEditableOrder((prev) => ({
      ...prev,
      delivery_datetime: formatted,
    }));
  };

  const value = editableOrder.delivery_datetime
    ? dayjs(editableOrder.delivery_datetime)
    : null;
    ...
    <StyledObserveOrder>
      ...
      <Info>
        ...
        <InfoItem className="half-row-2">
          ...
          {isEditMode && dayjs(editableOrder.delivery_datetime).isValid() ? (
            <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="fa">
              <DateTimePicker
                value={dayjs(editableOrder.delivery_datetime)}
                onChange={(newValue) => {
                  if (newValue && newValue.isValid()) {
                    const formatted = newValue.format("YYYY-MM-DD HH:mm:ss");
                    handleChange("delivery_datetime", formatted);
                  }
                }}
                ampm={false}
