...
function ObserveOrder({ order }) {
  ...
  const handleTimeChange = (e) => {
    const time = e.target.value; // مثلا "20:30"
    const datePart = editableOrder.delivery_datetime?.split(" ")[0]; // "2025-05-25"
    setEditableOrder((prev) => ({
      ...prev,
      delivery_datetime: `${datePart} ${time}`,
    }));
  };
          ...
          {isEditMode ? (
            <>
              <DatePicker
                ...
                onChange={(dateObj) => {
                  const gDate = dateObj.toDate(); // میلادی
                  const isoDateTime = gDate
                    .toISOString()
                    .slice(0, 16)
                    .replace("T", " ");
                  setEditableOrder((prev) => ({
                    ...prev,
                    delivery_datetime: isoDateTime,
                  }));
                }}
                format="YYYY/MM/DD"
                calendar={persian}
              ...
              <input
                type="time"
                value={
                  editableOrder.delivery_datetime?.split(" ")[1] || "12:00"
                }
                onChange={(e) => {
                  const date =
                  ...
                  setEditableOrder((prev) => ({
                    ...prev,
                    delivery_datetime: `${date} ${e.target.value}`,
                  }));
                }}
                className="ml-2"
              />
