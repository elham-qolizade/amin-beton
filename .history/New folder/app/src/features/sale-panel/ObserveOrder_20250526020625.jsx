...
function ObserveOrder({ order, onUpdate }) {
  ...
  useEffect(() => {
    ...
    (editableOrder.vibrators || []).forEach((vib, index) => {
      if (!vib.vibrator) {
        newErrors[`vibrator_${index}`] = `نوع ویبراتور شماره ${
          index + 1
        } باید انتخاب شود.`;
      }

      if (vib.count === "" || isNaN(vib.count) || vib.count < 1) {
        newErrors[`vibrator_count_${index}`] = `تعداد ویبراتور شماره ${
          index + 1
        } باید عددی بزرگتر از صفر باشد.`;
      }

      // اگر ویبراتور انتخاب شده و زیرمجموعه‌اش وجود داره اما انتخاب نشده
      if (vib.vibrator && vib.vibrator.has_sub_vibrator) {
        // فرض کردم که این فیلد وجود داره
        if (!vib.sub_vibrator) {
          newErrors[`sub_vibrator_${index}`] = `زیرمجموعه ویبراتور شماره ${
            index + 1
          } باید انتخاب شود.`;
        }
      }
