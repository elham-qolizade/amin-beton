...
function ObserveOrder({ order }) {
  ...
  // نگه‌داری لیست ساب پمپ‌ها برای هر pump.id
  const [subPumpDataByPumpId, setSubPumpDataByPumpId] = useState({});
  const [loadingPumpIds, setLoadingPumpIds] = useState([]);

  // بارگذاری subPump ها فقط وقتی مد ویرایش فعاله
  useEffect(() => {
    if (!isEditMode) return;

    const uniquePumpIds = [
      ...new Set(
        editableOrder.pumps.map((p) => p.pump?.id).filter((id) => !!id)
      ),
    ];

    uniquePumpIds.forEach(async (pumpId) => {
      if (subPumpDataByPumpId[pumpId]) return; // اگر قبلاً بارگذاری شده

      setLoadingPumpIds((prev) => [...prev, pumpId]);

      const { subPumpList } = await useSubPumpList(pumpId);
      setSubPumpDataByPumpId((prev) => ({
        ...prev,
        [pumpId]: subPumpList || [],
      }));

      setLoadingPumpIds((prev) => prev.filter((id) => id !== pumpId));
    });
  }, [editableOrder.pumps, isEditMode]);

  const getSubPumpList = (pumpId) => subPumpDataByPumpId[pumpId] || [];

  const isLoadingSubPump = (pumpId) => loadingPumpIds.includes(pumpId);
        ...
        <Pumps>
          {editableOrder.pumps.length === 0 ? (
            ...
            editableOrder.pumps.map((pump, index) => {
              ...
              const subPumpList = getSubPumpList(currentPumpId);
              const loadingSub = isLoadingSubPump(currentPumpId);
