      ...
      <Info>
        ...
        {editableOrder.vibrators.length === 0 ? (
          ...
          editableOrder.vibrators.map((vibrator, index) => {
            // ویبراتور اصلی از لیست کامل
            const mainVibrator = vibratorList.find(
              (v) => v.id === vibrator.vibrator
            );
            // زیرمجموعه های مرتبط با این ویبراتور اصلی
            const subVibrators = vibratorList.filter(
              (v) => v.parent === vibrator.vibrator
            );
            // زیر ویبراتور انتخاب شده
            ...
            return (
              <Vibrator key={index}>
                ...
                {editableOrder.vibrators.length === 0 ? (
                  ...
                  editableOrder.vibrators.map((vibrator, index) => {
                    const mainVibratorId = vibrator.vibrator;
                    const mainVibrator = vibratorList.find(
                      (v) => v.id === mainVibratorId
                    );
                    const subVibrators = vibratorList.filter(
                      (v) => v.parent === mainVibratorId
                    );
                    const selectedSub = subVibrators.find(
                      (sv) => sv.id === vibrator.sub_vibrator
                    );
                    ...
                    return (
                      <Vibrator key={index}>
                        ...
                        <InfoItem>
                          ...
                          {isEditMode ? (
                            <select
                              value={vibrator.vibrator || ""}
                              onChange={(e) => {
                                const newVibratorId =
                                  parseInt(e.target.value, 10) || null;
                                setEditableOrder((prev) => {
                                  const updated = [...prev.vibrators];
                                  updated[index] = {
                                    vibrator: newVibratorId,
                                    sub_vibrator: null, // ریست زیرمجموعه
                                    count: updated[index]?.count || 1,
                                  };
                                  return { ...prev, vibrators: updated };
                                });
                              }}
                            >
                              <option value="">انتخاب کنید</option>
                        ...
                        <InfoItem>
                          ...
                          {isEditMode ? (
                            <select
                              value={vibrator.sub_vibrator || ""}
                              onChange={(e) => {
                                const subId =
                                  parseInt(e.target.value, 10) || null;
                                setEditableOrder((prev) => {
                                  const updated = [...prev.vibrators];
                                  updated[index].sub_vibrator = subId;
                                  return { ...prev, vibrators: updated };
                                });
                              }}
                            >
                  ...
                  {isEditMode ? (
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const subId = parseInt(e.target.value, 10) || null;
                        updated[index].sub_vibrator = subId;
                        setEditableOrder((prev) => ({
                          ...prev,
                          vibrators: updated,
