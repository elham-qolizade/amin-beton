                      ...
                      <>
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const selectedPump = pumpList.find(
                              (p) => p.id === selectedId
                            );

                            const updated = [...editableOrder.pumps];
                            updated[index].pump = selectedPump || null;
                            updated[index].sub_pump = null; // پاک کردن زیرپمپ در صورت تغییر پمپ اصلی
                        ...
                        {subPumpOptions.length > 0 && (
                          ...
                          >
                            زیرپمپ‌ها:{" "}
                            {subPumpOptions.map((sp) => sp.title).join("، ")}
                          </div>
                        )}
                      </>
                    ) : (
                  ...
                  <InfoItem>
                    ...
                    {isEditMode ? (
                      <select
                        ...
                        onChange={(e) => {
                          const updated = [...editableOrder.pumps];
                          const selectedSubPumpId = parseInt(e.target.value);
                          updated[index].sub_pump =
                            subPumpOptions.find(
                              (sp) => sp.id === selectedSubPumpId
                            ) || null;
                          setEditableOrder((prev) => ({
                            ...prev,
                            pumps: updated,
