...
import {
  ...
  usePumpList,
  // useSubPumpList رو حذف می‌کنیم چون دیگه مستقیم استفاده نمی‌کنیم
} from "../../hooks/useDropdownsData";

import Spinner from "../../ui/Spinner";
import { useQueries } from "@tanstack/react-query"; // اضافه شده
...
function ObserveOrder({ order }) {
  ...
  // این قسمت مهمه: گرفتن همزمان همه subPumpList ها با useQueries
  const subPumpsQueries = useQueries({
    queries: editableOrder.pumps.map((pump) => ({
      queryKey: ["sub-pump-list", pump.pump?.id],
      queryFn: () => getSubPumpList(pump.pump?.id),
      enabled: !!pump.pump?.id,
    })),
  });
