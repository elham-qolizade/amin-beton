    ...
    <StyledObserveOrder>
      ...
      <Info>
        ...
        <InfoItem className="full-row">
          تاریخ مورد نظر برای بتن‌ریزی :{" "}
          {isEditing ? (
            <JalaliDatePicker
              value={editedOrder.pouring_date}
              onChange={(date) =>
                setEditedOrder((prev) => ({ ...prev, pouring_date: date }))
              }
            />
          ) : (
            <span>{convertGeorgianDateToJalali(order.pouring_date)}</span>
          )}
        </InfoItem>
