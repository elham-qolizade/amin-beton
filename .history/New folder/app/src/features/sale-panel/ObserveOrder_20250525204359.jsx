...
function ObserveOrder({ order }) {
  ...
  // تابع کمکی برای تبدیل از DatePicker به ISO string
  const convertPickerToISO = ({ year, month, day }) => {
              ...
              <DatePicker
                value={
                  editableOrder.delivery_datetime
                    ? parseDateToPicker(editableOrder.delivery_datetime)
                    : null
                }
                onChange={(newDate) => {
                  const iso = convertPickerToISO(newDate);
                  handleChange("delivery_datetime", iso);
                }}
                inputPlaceholder="تاریخ را انتخاب کنید"
                shouldHighlightWeekends
                locale="fa"
              />
              {/* برای انتخاب ساعت جداگانه: */}
              <input
                type="time"
                value={
                  editableOrder.delivery_datetime
                ...
                onChange={(e) => {
                  ...
                  const oldDate = new Date(
                    editableOrder.delivery_datetime || new Date()
                  );
                  oldDate.setHours(parseInt(hours));
                  oldDate.setMinutes(parseInt(minutes));
                  handleChange("delivery_datetime", oldDate.toISOString());
                }}
                style={{ marginTop: "8px" }}
              />
            </>
