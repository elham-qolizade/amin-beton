...
function ObserveOrder({ order }) {
  ...
  const { subPumpList } = useSubPumpList(editableOrder.pumps?.[0]?.pump?.id);
  const { subVibratorList } = useSubVibratorList(
    editableOrder.vibrators?.[0]?.vibrator?.id
  );
  ...
  const updatePump = (index, changes) => {
    ...
    setEditableOrder({ ...editableOrder, pumps: updated });
  };

  const updateVibrator = (index, changes) => {
    const updated = [...editableOrder.vibrators];
    updated[index] = { ...updated[index], ...changes };
    setEditableOrder({ ...editableOrder, vibrators: updated });
  };
  ...
  return (
    <StyledObserveOrder>
      ...
      <Info>
        ...
        <InfoItem>
          نام کاربری:
          <span>{editableOrder.project.user_username}</span>
        </InfoItem>
              ...
              <PumpItem
                ...
                onRemove={(index) =>
                  setEditableOrder((prev) => ({
                    ...prev,
                    pumps: prev.pumps.filter((_, i) => i !== index),
                  }))
                }
              />
            ))
            ...
            editableOrder.vibrators.map((v, i) => (
              <VibratorItem
                ...
                onRemove={(index) =>
                  setEditableOrder((prev) => ({
                    ...prev,
                    vibrators: prev.vibrators.filter((_, i) => i !== index),
                  }))
                }
              />
            ))
