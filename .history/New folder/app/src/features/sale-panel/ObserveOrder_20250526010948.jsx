...
function ObserveOrder({ order, onUpdate }) {
  ...
  const [selectedPumpIds, setSelectedPumpIds] = useState(
    editableOrder.pumps.map((p) => p.pump?.id || null)
  );

  // const { subPumpList = [], isLoadingSubPumpList } =
  // useSubPumpList(selectedPumpId);
  const fetchSubPumps = async (pumpId) => {
    const res = await fetch(`/api/sub-pumps/?parent_id=${pumpId}`);
    ...
    <StyledObserveOrder>
      ...
      <Info>
        ...
        {editableOrder.pumps.map((pump, index) => {
          const pumpId = selectedPumpIds[index];
          const { subPumpList = [], isLoadingSubPumpList } =
            useSubPumpList(pumpId);
                ...
                {isEditMode ? (
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      // آپدیت editableOrder.pumps
                      const selectedPumpObj =
                        pumpList.find((p) => p.id === selectedId) || null;
                      const updatedPumps = editableOrder.pumps.map((p, i) =>
                        i === index
                  ...
                  isLoadingSubPumpList ? (
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const updatedPumps = editableOrder.pumps.map((p, i) =>
                          i === index ? { ...p, sub_pump: selectedSubPump } : p
                        );
                        setEditableOrder((prev) => ({
                          ...prev,
                          pumps: updatedPumps,
        ...
        <button
          ...
          onClick={() => {
            ...
            setSelectedPumpIds((prev) => [...prev, null]);
          }}
        >
          + افزودن پمپ جدید
        </button>
                    ...
                    {isEditMode ? (
                      isLoadingSubPumpList ? (
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            const selectedSubPump =
                              getSubPumpListFor(currentPumpId).find(
                                (sp) => sp.id === parseInt(e.target.value)
                              ) || null;
                        ...
                        >
                          <option value="">ندارد</option>
                          {getSubPumpListFor(currentPumpId).map((sp) => (
                            <option key={sp.id} value={sp.id}>
                              {sp.title}
                            </option>
                          ))}
