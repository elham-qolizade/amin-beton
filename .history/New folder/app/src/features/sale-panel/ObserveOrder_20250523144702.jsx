...
// فرض بر این که این تابع async رو دارید که لیست ساب‌پمپ‌ها رو می‌گیره
// import { getSubPumpList } from "../../api";
...
function ObserveOrder({ order }) {
  ...
  // ذخیره لیست ساب‌پمپ‌ها بر اساس pumpId
  const [subPumpLists, setSubPumpLists] = useState({});

  useEffect(() => {
    editableOrder.pumps.forEach(async (pump) => {
      if (pump.pump?.id && !subPumpLists[pump.pump.id]) {
        try {
          const list = await getSubPumpList(pump.pump.id);
          setSubPumpLists((prev) => ({
            ...prev,
            [pump.pump.id]: list,
          }));
        } catch (error) {
          console.error("خطا در گرفتن ساب پمپ‌ها:", error);
        }
      }
    });
  }, [editableOrder.pumps, subPumpLists]);
        ...
        <Pumps>
          {editableOrder.pumps.length === 0 ? (
            ...
            editableOrder.pumps.map((pump, index) => {
              const selectedPumpId = pump.pump?.id;
              const subPumpList = subPumpLists[selectedPumpId] || [];
