...
function ObserveOrder({ order, onUpdate }) {
  ...
  const jalaliDate = moment("2025-05-26T10:50").format("jYYYY/jMM/jDD HH:mm");

  // تبدیل تاریخ شمسی به میلادی برای ذخیره:
  const miladiDate = moment
    .from("1404/03/05 10:50", "fa", "jYYYY/jMM/jDD HH:mm")
    .toDate();
  ...
  useEffect(() => {
    if (editableOrder?.delivery_datetime) {
      // فرض: مقدار از سرور به صورت میلادی میاد
      const jDate = moment(editableOrder.delivery_datetime).format(
        "jYYYY/jMM/jDD HH:mm"
      );
      setValue(jDate);
    }
  ...
  useEffect(() => {
    if (editableOrder?.delivery_datetime) {
      setValue(
        new DateObject({
          date: editableOrder.delivery_datetime, // مقدار شمسی به صورت رشته
          format: "YYYY/MM/DD HH:mm", // حتماً ست شود
          calendar: persian,
          locale: persian_fa,
        })
      );
        ...
        <InfoItem className="half-row-2">
          ...
          {isEditMode ? (
            <DatePicker
              value={value}
              onChange={(val) => {
                // 1. مقدار انتخاب‌شده به‌صورت شمسی (DateObject)
                setValue(val);

                // 2. تبدیل به رشته‌ی شمسی
                const jalaliString = val?.format("YYYY/MM/DD HH:mm");

                // 3. تبدیل به میلادی با moment-jalaali
                const miladiDate = moment
                  .from(jalaliString, "fa", "jYYYY/jMM/jDD HH:mm")
                  .toDate();

                // 4. ذخیره مقدار میلادی به backend
                handleChange("delivery_datetime", miladiDate);
              }}
              calendar={persian}
              locale={persian_fa}
