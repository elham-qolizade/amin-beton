...
function ConfirmedOrders() {
  ...
  <DatePicker
    ...
    onChange={(date) => {
      if (date) {
        const jDate = date.format("YYYY-MM-DD"); // تاریخ شمسی
        const gDate = moment(jDate, "jYYYY-jMM-jDD").format("YYYY-MM-DD"); // تبدیل به میلادی
        setStartDate(gDate); // برای ارسال به بک‌اند
      } else {
        setStartDate("");
      }
    }}
