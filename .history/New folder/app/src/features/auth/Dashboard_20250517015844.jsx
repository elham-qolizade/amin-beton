...
export default function Dashboard() {
  ...
  return (
    <div className="p-8  flex justify-center items-center flex-col mx-auto">
      <h1 className="text-3xl font-bold text-center mb-6 text-blue-700">
        داشبورد
      </h1>
      <ul className="grid grid-cols-1 max-w-4xl  sm:grid-cols-2 gap-4 mb-8">
        {allowedPanels.map((panel) => (
          <li
            key={panel.role}
            onClick={() => navigate(panel.path)}
            className="cursor-pointer p-4 bg-blue-100 hover:bg-blue-200 rounded-xl shadow transition-all duration-300 text-center text-lg font-medium"
          >
            {panel.name}
          </li>
        ))}
